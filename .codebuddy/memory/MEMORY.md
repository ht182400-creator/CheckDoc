...
- **P2 (v0.4, 11.5h) 🔄 进行中**：✅ treemap 图谱（M3）、✅ Excel 导出、✅ 目录分组筛选、✅ 时间范围筛选、✅ 多记录抽取（按##拆分）、✅ 统计面板重构、✅ 交互式候选面板（点击加入词库）、✅ 候选发现修复、✅ 目录浏览、✅ 筛选栏紧凑化、✅ 表格分页、✅ CSV/MD 导出修复（bytes+BOM）、✅ 测试债务清零（v0.0.4，verify 全绿）；📋 LLM 质量评分、📋 定时同步、📋 代码高亮
