...
## 当前版本
- v0.0.8（已推送 GitHub，新增 PDF 导出 / 记录内编辑·修正低分 / K3 可选 LLM 后端，loop/verify.py 全绿）

## 停止条件（P2）：✅ 已满足（P2 四项全完成 + 测试绿 + 文档同步 + 已推送）
> 每日 09:00 自动化将在下次触发时读到本看板 P2 全绿而**自动停止**；如需立即停，可手动将 automation 设为 PAUSED。
> 注：v0.0.8 的 P3 三项为**用户手动追加需求**（非每日自动化范围），已随本轮实现并推送。若要让自动化继续演进，需更新"停止条件"纳入 P3+。
